...
class SuperNotificationService(BaseNotificationService):
    ...
    async def initialize(self):
            ...
            if r.get(CONF_MOBILE_DISCOVERY):
                ...
                if r.get(CONF_MOBILE_DEVICES):
                    _LOGGER.info(
                        ...
                        r[CONF_PERSON],
                        r[CONF_MOBILE_DEVICES],
                    )
                else:
                    _LOGGER.warning(
                        "SUPERNOTIFY Unable to find mobile devices for %s",
                        r[CONF_PERSON],
                    )
        ...
        unknown_deliveries = {
            ...
            if dc.get(CONF_METHOD) not in method_names or d in RESERVED_DELIVERY_NAMES
        }

        if unknown_deliveries:
            _LOGGER.info(
                "SUPERNOTIFY Ignoring deliveries without known methods: %s",
                unknown_deliveries,
            )

        _LOGGER.info(
            "SUPERNOTIFY configured deliveries %s", ";".join(self.deliveries.keys())
        )
                        ...
                        mobile_devices.append(
                            {
                                ...
                                CONF_NOTIFY_SERVICE: "mobile_app_%s"
                                % slugify(device.name),
